                ...
                <div class="card border-0 shadow rounded">
                    <div class="card-body">
                        <x-jet-button class="ml-10 mt-8" style=" background-color:rgb(40,155,255); height:40px;  margin-top:-100px; margin-bottom:10px;">
                            <a href="{{ route('branch.create') }}" style="color:white;"><i class="fas fa-user-plus" style="margin-left:-5px;"> </i> ADD  
                    </a>  </x-jet-button> 
                              ...
                              @forelse ($branches as $branch)
                                <tr>
                                    ...
                                    <td class="border" style="border-radius:200% " width="100px" height="100px">
                                        <button style="background-color:blue; width:35px; border-radius:10px; margin-top:3px; display:inline-block;"><a href="{{ route('branch.edit', $branch->id) }}" style="color:white;"><i class="fas fa-edit"></i></a></button>
                                          
                                        <form onsubmit="return confirm('Apakah Anda Yakin ?');" action="{{ route('branch.destroy', $branch->id) }}" method="POST">
                                            @csrf
                                            @method('DELETE')
                                            <button style="background-color:red; width:35px; border-radius:10px; margin-top:3px; display:inline-block;"><a href="{{ route('branch.destroy', $branch->id) }}" style="color:white;"><i class="fas fa-trash-alt"></i></a> </button>
                                        </form>
                                    </td>
                                </tr>
                                @empty
                                <div class="alert alert-danger">
                                    Data Branches belum Tersedia.
                                </div>
                            @endforelse
                          </tbody>
                        </table>
                        <div class="container mt-5">
                            <div class="container mt-5">
                                {!! $branches->appends(['sort' => 'science-stream'])->links() !!}
                            </div>
                        </div>
                            </div>
                        </div>
